...
for _ in 0..<N {
    let jewelInput = readLine()!.split(separator: " ").map { Int(String($0))! }
    arr.append((m : jewelInput[0] , v : jewelInput[1]))
}

for _ in 0..<K {
    let bagInput = Int(readLine()!)!
    bagArr.append(bagInput)
}
...
var heap = Heap<(Int , Int)>(array: []) {
    $0.1 > $1.1
}

var idx = 0

for bagWeight in bagArr {
    while idx < arr.count && arr[idx].m <= bagWeight {
        heap.insert(arr[idx])
        idx += 1
    }
